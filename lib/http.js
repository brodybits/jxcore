...
var uw = process.binding('jxutils_wrap');
var _jx_helper = require('_jx_http_helper');
var pin = _jx_helper.pinIt;
...
exports.IncomingMessage = require('_jx_incoming').IncomingMessage;
var parsers = require('_jx_parsers').parsers;
exports.parsers = parsers;

var CRLF = '\r\n';
var STATUS_CODES = _jx_helper.STATUS_CODES;
...
var dateCache, oldDateCache = Date.now();
var utcDate = function() {
  var newDate = Date.now();
  if (!dateCache || newDate > oldDateCache + 500) {
    var d = new Date();
    dateCache = d.toUTCString();
    oldDateCache = newDate;
  }
...
function OutgoingMessage() {
  ...
  this.outputEncodings = new Array();
...
OutgoingMessage.prototype._storeHeader = function(firstLine, headers) {
  ...
  if (headers) {
    ...
    for (var i = 0, l = keys.length; i < l; i++) {
      ...
      if (Array.isArray(value)) {
        for (var j = 0, lnn = value.length; j < lnn; j++) {
  ...
  // pin("OutgoingMessage.prototype._storeHeader-end");
};
...
function chunkify(chunk, headers, trailers, last) {
  ...
  if (chunklen & 0xf0000000)
    octets += 7;
  else if (chunklen & 0xf000000)
    octets += 6;
  else if (chunklen & 0xf00000)
    octets += 5;
  else if (chunklen & 0xf0000)
    octets += 4;
  else if (chunklen & 0xf000)
    octets += 3;
  else if (chunklen & 0xf00)
    octets += 2;
...
OutgoingMessage.prototype.end = function(data, encoding) {
  ...
  var ln = 0, lnbuffer = false;
  if (data) {
    ln = data.length;
    lnbuffer = Buffer.isBuffer(data);
  }
  ...
  if (hot && lnbuffer && ln > BIG_CHUNK) hot = false;

  // Can't concatenate safely with hex or base64 encodings.
  if (encoding === 'hex' || encoding === 'base64') hot = false;

  if (hot) {
    if (!lnbuffer && typeof data === 'string') {
      if (this.chunkedEncoding) {
...
ServerResponse.prototype.assignSocket = function(socket) {
  // pin("ServerResponse.prototype.assignSocket");
...
ServerResponse.prototype.detachSocket = function(socket) {
  // pin("ServerResponse.prototype.detachSocket");
...
ServerResponse.prototype.writeContinue = function() {
  // pin("ServerResponse.prototype.writeContinue");
...
ServerResponse.prototype.writeHead = function(statusCode, aa, bb) {
  var reasonPhrase, headers, obj;
  ...
  if (obj && this._headers) {
    ...
    if (Array.isArray(obj)) {
      ...
      for (var i = 0, lnn = keys.length; i < lnn; i++) {
        var k = keys[i];
...
Agent.prototype.removeSocket = function(s, name, host, port,
    localAddress) {// pin("Agent.prototype.removeSocket");
  if (this.sockets[name]) {
...
ClientRequest.prototype._implicitHeader = function() {
  // pin("ClientRequest.prototype._implicitHeader");
  this._storeHeader(this.method + ' ' + this.path + ' HTTP/1.1\r\n',
                    this._renderHeaders());
};

ClientRequest.prototype.abort = function() {
  // pin("ClientRequest.prototype.abort");
...
function createHangUpError(n) {// pin("createHangupError");
  var error = new Error('socket hang up -' + n);
...
function socketCloseListener() {
  ...
  if (req.res && req.res.readable) {
    ...
    // fire on the request.
    req.emit('error', createHangUpError(1));
    req._hadError = true;
  }
...
function socketOnEnd() {
  ...
  if (!req.res) {
    ...
    req.emit('error', createHangUpError(2));
    req._hadError = true;
  }
  if (parser) {
    parser.finish();
...
function socketOnData(d, start, end) {
  ...
  var ret = parser.execute(d, start, end - start, parser.__ptype);
  parser.__ptype = null;
...
// client
function parserOnIncomingClient(res,
    shouldKeepAlive) {// pin("parseOnIncomingClient");
  var parser = this;
...
// client
function responseOnEnd() {// pin("responseOnEnd");
  var res = this;
  var req = res.req;
  ...
  if (!req.shouldKeepAlive) {
    ...
    process.nextTick(function() {// pin("goFree");
      socket.emit('free');
    });
  }
}
...
ClientRequest.prototype.onSocket = function(socket) {
  // pin("ClientRequest.prototype.onSocket");
  var req = this;

  process.nextTick(function() {// pin("onSocketnextTick");
    var parser = parsers.alloc();
    req.socket = socket;
    req.connection = socket;
    parser.__ptype = HTTPParser.RESPONSE; // null; pin("RESPONSE - reinit");
...
ClientRequest.prototype._deferToConnect = function(method, arguments_,
    cb) {// pin("_deferToConnect");
  ...
  // eventually writable).
  var self = this;
  var onSocket = function() {// pin("xonSocket");
    if (self.socket.writable) {
      ...
      self.socket.once('connect', function() {// pin("onceconnect");
        if (method) {
          self.socket[method].apply(self.socket, arguments_);
        }
        if (cb) {
...
ClientRequest.prototype.setTimeout = function(msecs,
    callback) {// pin("ClientRequest.prototype.setTimeout");
...
function httpSocketSetup(socket) {// pin("httpSocketSetup");
  socket.removeListener('drain', ondrain);
  socket.on('drain', ondrain);
}

function Server(requestListener) {// pin("Server");
...
exports.setMaxHeaderLength = function(size) {
  if (size < 1024 && size != 0) {
    throw new RangeError(
        'HTTP header size can not be smaller than 1024 bytes. ' +
        'Set 0 to disable');
  }

  maxHeaderSize = size;
  uw.setMaxHeaderLength(size);
};

var maxHeaderWarningShown = false;
var maxHeaderSize = 0; // commons.h
...
function connectionListener(socket) {
  ...
  function serverSocketCloseListener() {
    ...
    if (this.parser) freeParser(this.parser);

    if (incoming.length) abortIncoming();
  }
  ...
  if (self.timeout) socket.setTimeout(self.timeout);
  ...
  var parser = parsers.create(HTTPParser.REQUEST, socket);
  socket.parser = parser;
  ...
  socket.ondata = function(d, start, end, parser_response) {
    var ret;
    if (parser_response === undefined) {
      assert(!socket._paused);
      ret = parser.execute(d, start, end - start, parser.__ptype);
    } else {
      ret = parser_response;
    }
    ...
    if (ret == -2) {
      if (!maxHeaderWarningShown) {
        var err =
            'Request is denied for security reasons. ' +
            'Maximum allowed header size is configured to ' + maxHeaderSize +
            ' bytes. You may replace it from http.setMaxHeaderLength';
        debug(err);
        console.error(err);
        maxHeaderWarningShown = true;
      }
      socket.destroy(ret);
      return;
    }
