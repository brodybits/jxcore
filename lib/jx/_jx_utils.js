      ...
      try {
        ...
        if (!(/[\/\\.]/.test(a))) {
          ...
          if (ret.exitCode == 0) {
            ...
            exports.console.log(
                'When embedded, this feature expects JXcore available via ' +
                "'jx' on the system path", 'yellow');
          }
        }
        return _this.req(a, b, c, d);
      }
...
exports.jump = function() {
  if (!jxutil.runLoop(-1)) {
    console.error(
        "jump interrupted. Make sure the thread wasn't paused already");
  }
};
...
exports.getCPU = function(cb, timer) {
  if (checkInter && cb) {
    throw new Error('You can not run multiple instances of getCPU. ' +
        'Call getCPU() without parameters to shutdown the previous one');
  }
...
var removeColors = function(txt) {
  for (var o in ANSI_CODES) {
    ...
    var first = ANSI_CODES[o][0];
    var second = ANSI_CODES[o][1];

    if (!ANSI_CODES_REGEX[o]) {
      ANSI_CODES_REGEX[o] = [];
      if (first)
        ANSI_CODES_REGEX[o].push(
            new RegExp('\\' + first.replace('[', '\\['), 'g'));

      if (second)
        ANSI_CODES_REGEX[o].push(
            new RegExp('\\' + second.replace('[', '\\['), 'g'));

    }

    if (first) txt = txt.replace(ANSI_CODES_REGEX[o][0], '');
    if (second) txt = txt.replace(ANSI_CODES_REGEX[o][1], '');
  }
...
exports.console.clear = function() {
  if (!arguments.length) {
    process.stdout.write(ANSI_CODES.clear[0]);
    return;
  }
  ...
  if (!color) {
    arguments.length++;
    arguments[arguments.length - 1] = 'clear';
  } else {
    // clear should be always in front
    color = color.replace(/clear/g, '').replace(/\+\+/g, '+').trim();
    if (!color)
      color = 'clear';
    else
      color = 'clear+' + color;

    arguments[arguments.length - 1] = color;
  }
  console.log(setColor.apply(null, arguments));
};
